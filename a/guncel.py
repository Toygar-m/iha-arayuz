...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.gridLayoutWidget_4.setGeometry(QtCore.QRect(1540, 380, 351, 91))
    ...
    def guncelle(self):
        ...
        for i in range(len(boxes)):
            if i in cv2.dnn.NMSBoxes(boxes, confidences, 0.4, 0.3):
                x, y, w, h = boxes[i]
                # color = np.random.uniform(0, 255, size=(len(self.classes), 3))[class_ids[i]]
                color = (0, 0, 255)
                cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
                cv2.rectangle(frame, (x, y), (x + w, y + 30), color, -1)
                cv2.putText(frame, str(self.classes[class_ids[i]]) + " " + str(round(confidences[i], 2)), (x, y + 30), cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 255), 3)

        elapsed_time = time.time() - self.starting_time
        fps = self.frame_id / elapsed_time
        cv2.putText(frame, "FPS: " + str(round(fps, 2)), (10, 50), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 0), 3)

        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        h, w, ch = rgb_frame.shape
        bytes_per_line = ch * w
        self.label_38.setPixmap(QtGui.QPixmap.fromImage(QtGui.QImage(rgb_frame.data, w, h, bytes_per_line, QtGui.QImage.Format_RGB888)))
