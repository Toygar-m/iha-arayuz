...
class Ui_MainWindow(object):
    def __init__(self):
        ...
        self.current_fps = 0
        self.image_data = None  # İşlenmiş görüntü için değişken
    ...
    def yolo_process(self):
        ...
        while True:
            ...
            # İşlenmiş görüntüyü ana iş parçacığına ilet
            self.processed_queue.put(processed_frame)
            self.fps_queue.put(1 / (time.time() - self.starting_time))
            self.starting_time = time.time()

    def guncelle(self):
        # Eğer işlenmiş bir görüntü varsa, GUI'yi güncelle
        if not self.processed_queue.empty():
            ...
            # Görüntüyü RGB'ye çevir ve QLabel'e aktarılabilir hale getir
            rgb_frame = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)
            h, w, ch = rgb_frame.shape

            # İşlenmiş görüntüyü GUI'ye aktarma
            self.image_data = QtGui.QImage(rgb_frame.data, w, h, ch * w, QtGui.QImage.Format_RGB888)
            self.label_38.setPixmap(QtGui.QPixmap.fromImage(self.image_data))

    async def websocket_client(self):
        uri = "ws://192.168.31.252:6789"  # WebSocket sunucu adresi
        async with websockets.connect(uri) as websocket:
            while True:
                message = await websocket.recv()
                data = json.loads(message)

                # Base64'dan görüntü verisini çöz
                frame = cv2.imdecode(np.frombuffer(base64.b64decode(data["image"]), np.uint8), cv2.IMREAD_COLOR)
                if not self.frame_queue.full():
                    self.frame_queue.put(frame)

# WebSocket istemcisini başlatan asenkron fonksiyon
async def start_websocket(ui):
    await ui.websocket_client()

# Qt ile asyncio'yu uyumlu hale getirmek için QEventLoop kullanma
def start_asyncio_loop(ui):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)  # Yeni bir event loop oluşturuyoruz
    loop.create_task(start_websocket(ui))
    loop.run_forever()
...
if __name__ == "__main__":
    ...
    # Qt ve asyncio'yu uyumlu hale getirmek için ayrı bir iş parçacığında asyncio'yu başlatma
    asyncio_thread = Thread(target=start_asyncio_loop, args=(ui,))
