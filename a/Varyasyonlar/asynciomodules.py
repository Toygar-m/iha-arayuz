...
class Ui_MainWindow(object):
    def __init__(self):
        ...
        self.fps_queue = asyncio.Queue(maxsize=1)
    ...
    async def process_frames(self):
        while True:
            ...
            processed_frame = await self.run_yolo(frame)

            end_time = time.time()
            self.current_fps = 1 / (end_time - start_time)

            await self.processed_queue.put(processed_frame)
            await self.fps_queue.put(self.current_fps)

    async def run_yolo(self, frame):
        return await asyncio.to_thread(self.model_inference, frame)
    ...
    def guncelle(self):
        ...
        if self.frame_queue.qsize() < 1:
            asyncio.ensure_future(self.frame_queue.put(frame))

        if self.processed_queue.qsize() > 0:
            processed_frame = self.processed_queue.get_nowait()
            if self.fps_queue.qsize() > 0:
                self.current_fps = self.fps_queue.get_nowait()

            cv2.putText(processed_frame, f"FPS: {self.current_fps:.2f}", (10, 50),
